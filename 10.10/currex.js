...
$(document).ready(function () {
   ...
   // looping the the exchangeRates object 
   //matching with allCurrencies object
   //to create option of different currency
   for (let currency in exchangeRates.rates) {
      $("#toCurrency").append(`<option value = ${currency} >${allCurrencies[currency]} (${currency})</option>`);
   }

   //part 2
   // when use use drop down it trigger this event handler
   $("#toCurrency").on("change", function () {
      // dollar value
      let usdollar = $("#usdInput").val();
      // chosen option 
      let selectedCurrency = $(this).val();
      // conveting us dollar to whatever
      // 2 fix point
      let convertedAmount = (usdollar * exchangeRates.rates[selectedCurrency]).toFixed(2)
      //show the covernt number in input text box
      $('#resultCurrency').val(convertedAmount);
      // change lab name
      $('#resultLabel').text(`${allCurrencies[selectedCurrency]} (${selectedCurrency}):`);
   });
   //part 3
   //update handler
   $('#updateRates').on("click", function () {
      //update exchangeRates objects
      exchangeRates = JSON.parse($('#exchangeRates').val())
      // select the drop down menu
      //and delete everything
      let select = $("#toCurrency");
      select.empty();
      //non option text
      $("#toCurrency").append(`<option value="" disabled selected>Select currency</option>`);
      // looping the the exchangeRates object 
